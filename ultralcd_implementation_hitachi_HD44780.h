...
unsigned long secCounterTimeout = 0;
...
FORCE_INLINE void lcd_print_space(int n)
{
	while (n--)
		lcd.print(' ');
}
...
static void lcd_implementation_status_screen()
{
	...
	if (secCounter%2 == 0)
	{
		...
		lcd.print('/');
		lcd.print(itostr3(tTarget1));
		lcd_printPGM(PSTR(LCD_STR_DEGREE " "));
	}
	if (millis() - secCounterTimeout > 1000)
	{
		secCounterTimeout = millis();
...
# ifdef TEMP_SENSOR_BED != 0
	...
	int left = 12 - strlen(lcd_filename) - 1;
	lcd.setCursor(0, 2);
