...
bool Z_MIN_ENDSTOP_INVERTING = true ; // MOKO ATOM true , set to true to invert the logic of the endstop. 
...
void enquecommand_P(const char *cmd, bool sd)
{
  if(buflen < BUFSIZE)
  {
	cmds[bufindw].clear();
	char c;
	while ((c = pgm_read_byte(cmd++)) != NULL)
	{
		cmds[bufindw].add(c);
	}
	cmds[bufindw].save_value();
...
void calibrate_print_surface() {
  ...
  for (int y = 0; y < 7; y++) 
  {
    for (int x = 0; x < 7; x++) 
	{
	  if (x != 3 || y != 3)
      {
         bed_level[x][y] -= bed_level[3][3];
      }
    }
  }
  home_pos[Z_AXIS] = MANUAL_Z_HOME_POS - bed_level[3][3];
  bed_level[3][3] = 0;
}
...
FORCE_INLINE void process_mcode(int value)
{
	...
	case 104: // M104
		...
		if (code_seen('S'))
		{
			SERIAL_ECHO("temp=");SERIAL_ECHO(tmp_extruder);SERIAL_ECHO("=");
	...
	case 109: // M109 - Wait for extruder heater to reach target.
	{
		...
		LCD_MESSAGEPGM(MSG_NOZZLE_HEATING);
#ifdef AUTOTEMP
...
void prepare_move()
{
      //servos[0].write(65) ; //change to T0 by cxam5 , 20171229
      //delay(1000); // change to T0 by cxam5 , 20171229
      //servos[0].write(75); //change to T0 by cxam5 , 20171229
  previous_millis_cmd = millis();
  float diff[NUM_AXIS];
  int start = 0;
  ...
  for (int s = 1; s <= steps; s++) 
  {
    ...
    plan_buffer_line(delta[X_AXIS], delta[Y_AXIS], delta[Z_AXIS],
                     destination[E_AXIS], real_feedrate/6000.0,
                     active_extruder);
  }
  for(int8_t i=0; i < NUM_AXIS; i++) 
  {
